    ...
    for (int i_iter = 0; i_iter < n_iter; i_iter ++)
    {
        ...
        {   
            ...
            {   
                ...
                if (i_view % P == 0)
                {
                    host_deform_invert(d_img1, d_img_temp, nx, ny, nz, volumes[i_view] - volumes[0], flows[i_view] - flows[0], d_alpha_x, d_alpha_y, d_alpha_z, d_beta_x, d_beta_y, d_beta_z);
                ...
                else
                {
                    ...
                    host_deform_invert(d_img_temp, d_img1, nx, ny, nz, volumes[iv1] - volumes[0], flows[iv1] - flows[0], d_alpha_x, d_alpha_y, d_alpha_z, d_beta_x, d_beta_y, d_beta_z);
                    host_add(d_img, d_img_temp, nx, ny, nz, lambda);
                }
            }
            cudaMemcpy(h_outimg, d_img, numBytesImg, cudaMemcpyDeviceToHost);
