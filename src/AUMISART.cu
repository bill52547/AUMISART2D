    ...
    for (int i_iter = 0; i_iter < n_iter; i_iter ++)
    {
        ...
        {   
            ...
            {   
                ...
                stat = cublasSnrm2(handle, na, d_proj, 1, &tempNorm0);

                host_add(d_proj, d_proj_temp, na, 1, -1.0);
                stat = cublasSnrm2(handle, na, d_proj, 1, &tempNorm);
    ...
    for (int i_iter = 0; i_iter < n_iter; i_iter ++)
    {
        ...
        {   
            ...
            {   
                ...
                if (i_view % P == 0)
                {
                    host_deform(d_img1, d_img, nx, ny, volumes[i_view] - volumes[0], flows[i_view] - flows[0], d_alpha_x, d_alpha_y, d_beta_x, d_beta_y);
                    host_projection(d_proj_temp, d_img1, angles[i_view], SO, SD, da, na, ai, nx, ny);
                }
                else 
                {
                    host_deform(d_img1, d_img, nx, ny, volumes[iv1] - volumes[0], flows[iv1] - flows[0], d_alpha_x, d_alpha_y, d_beta_x, d_beta_y);
                ...
                if (i_view % P == 0)
                {
                    host_deform_invert(d_img1, d_img_temp, nx, ny, volumes[i_view] - volumes[0], flows[i_view] - flows[0], d_alpha_x, d_alpha_y, d_beta_x, d_beta_y);
                ...
                else
                {
                    ...
                    host_deform_invert(d_img_temp, d_img1, nx, ny, volumes[iv1] - volumes[0], flows[iv1] - flows[0], d_alpha_x, d_alpha_y, d_beta_x, d_beta_y);
                    host_add(d_img, d_img_temp, nx, ny, lambda);
                }
            }
            cudaMemcpy(h_outimg, d_img, numBytesImg, cudaMemcpyDeviceToHost);
